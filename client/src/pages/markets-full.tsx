...
export default function MarketsFullPage() {
  ...
  const fetchMarketData = async () => {
    try {
      ...
      // Try the Direct Binance API first using the official Binance connector SDK (more reliable for geo-restricted regions)
      let response = await fetch('/api/direct-binance/top-pairs');
      
      // If Direct Binance API fails, try Python-based Binance SDK
      if (!response.ok) {
        console.log('Direct Binance API failed, trying Python Binance SDK endpoint');
        response = await fetch('/api/markets/python-binance/prices');
        
        // If Python-based Binance SDK fails, fall back to the standard API
        if (!response.ok) {
          console.log('Python Binance SDK failed, falling back to standard API');
          response = await fetch('/api/binance/all-markets');
          
          if (!response.ok) {
            const errorData = await response.json().catch(() => ({}));
            throw new Error(errorData?.message || 'Failed to fetch market data from Binance API');
          }
        }
      }
