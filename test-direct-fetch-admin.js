/**
 * Direct fetch-based Admin Check Test
 * 
 * This script uses native Node.js fetch API to bypass middleware issues.
 */
  ...
  try {
    ...
    const createResponse = await fetch('http://localhost:5000/api/auth/create-default-admin', {
      ...
      headers: {
        ...
        'X-Direct-API': 'true' // Special header to bypass Vite
      }
    });
...
async function checkAdminUser(email) {
  try {
    console.log(`\n2. Checking admin user: ${email}`);
    
    const checkResponse = await fetch(`http://localhost:5000/direct-api/admin/user?email=${email}`, {
      method: 'GET',
      headers: {
        'Accept': 'application/json',
        'Content-Type': 'application/json',
